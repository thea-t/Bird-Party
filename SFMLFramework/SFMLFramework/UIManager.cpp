...
UIManager::UIManager()
{
	...
	m_levelPassedText.setOrigin( textRect.left + textRect.width / 2.0f, textRect.top + textRect.height / 2.0f );
...
void UIManager::draw( sf::RenderWindow* pWindow, GameState* gameState )
{
	...
	// check which game state is active and draw the UI elements accordingly.
...
{
	// check if which game state is currently active.
	if ( *gameState == GameState::Menu ) 
	{
		// animate the menu text to scale up and down.
		...
		// check if space button is pressed. If so, go to the Play scene and change the music
	...
	else if ( *gameState == GameState::End ) 
	{
		...
		m_youDiedText.setOrigin( textRect.left + textRect.width / 2.0f, textRect.top + textRect.height / 2.0f );
